...
def showstr(s_str, n=16):
    for x in s_str[:n]:
        print ('%02x' % ord(x))

def getNumber(s_str, length):
    # MIDI uses big-endian for everything
    s_sum = 0
    for i in range(length):
        s_sum = (s_sum << 8) + s_str[i]
    return s_sum, s_str[length:]

def getVariableLengthNumber(s_str):
    s_sum = 0
    i = 0
    while 1:
        x = s_str[i]
        i = i + 1
        s_sum = (s_sum << 7) + (x & 0x7F)
        if not (x & 0x80):
            return s_sum, s_str[i:]
...
class MidiEvent:
    ...
    def __cmp__(self, other):
        ...
        return self.time is not None and other.time is not None
    def __repr__(self):
...
class MidiFile:
    ...
    def writestr(self):
        ...
        s_str = "MThd" + putNumber(6, 4) + putNumber(self.format, 2)
        s_str = s_str + putNumber(len(self.tracks), 2)
        s_str = s_str + putNumber(division, 2)
        for trk in self.tracks:
            s_str = s_str + trk.write()
        return s_str
