...
from music_objects import TimeSignature, BarList, NoteSequence, Bar, Note
from basic_functions import calculate_bar_ticks
...
def get_bars_from_midi(midiFile):
    ...
    def get_initial_tempo(timeList):
        ...
        while(tempo is None and i<len(timeList)):
            event = timeList[i]
            i = i + 1
    ...
    def get_initial_time_signature(timeList):
        ...
        i=0
        # Find the initial time and tempo:
        while(timesig is None and i<len(timeList)):
            event = timeList[i]
            i = i + 1
