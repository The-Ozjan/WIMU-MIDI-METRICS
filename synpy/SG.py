...
from basic_functions import get_H, velocity_sequence_to_min_timespan, get_rhythm_category, find_rhythm_Lmax
        ...
        if not are_parameters_valid(Lmax, weightSequence, subdivisionSequence):
            ...
            if Lmax is not None:
                ...
                def ave_dif_neighbours(index, level):
                    ...
                    for i in range(level, max(H)+1):
                        ave = (parameterGarma*dif(index,find_pre(index,i))+dif(index,find_post(index,i)) )/(1+parameterGarma)
                        averages.append(ave)
                    return averages
