...
def get_cost(sequence,nextSequence):
    ...
    if sequence[1:] == repeat([0],len(sequence)-1):        # null prototype
        ...
        cost = 2
    elif sequence[0] == 1 and (nextSequence is None or nextSequence[0] == 0):    # run2 prototype
        cost = 2
    elif sequence[-1] == 1 and nextSequence is not None and nextSequence[0] == 1:        # upbeat prototype
        cost = 3
...
def syncopation_perlevel(subSequences):
    ...
    for i in range(len(subSequences)-1):
        ...
        total = total + get_cost(subSequences[i], subSequences[i+1])
