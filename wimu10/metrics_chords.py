...
def get_chords_list(track: mp.Track, error_frame: int = 75) -> list[list[int]]:
    """
...
def _piano_chords(chords_list: list[list[int]]) -> list[str]:
    return ['/'.join([_map_midi_to_piano(midi_number) for midi_number in chord]) for chord in chords_list]
